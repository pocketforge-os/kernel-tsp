	...
	if (bluesleep_can_sleep()) {
		...
		if (bsi->uport->ops->tx_empty(bsi->uport)) {
			...
			if (gpio_get_value(bsi->ext_wake) == 1)
				BT_SLEEP_DBG("AP_WAKE_BT high");
			else
				BT_SLEEP_DBG("AP_WAKE_BT low");

			if (gpio_get_value(bsi->host_wake) == 1)
				BT_SLEEP_DBG("BT_WAKE_AP high");
			else
				BT_SLEEP_DBG("BT_WAKE_AP low");
...
static void bluesleep_outgoing_data(void)
{
	...
	BT_SLEEP_DBG("host wake bt(%d,%d),bt wake host(%d,%d)",
		bsi->bt_wake_polarity, gpio_get_value(bsi->ext_wake),
		bsi->host_wake_polarity, gpio_get_value(bsi->host_wake));
...
static int bluesleep_start(void)
{
	...
	BT_ERR("%s: call enable_irq_wake for irq: %d\n", __func__, bsi->host_wake_irq);
...
static void bluesleep_stop(void)
{
	...
	BT_ERR("%s: call disable_irq_wake for irq: %d\n", __func__, bsi->host_wake_irq);
...
static int bluesleep_probe(struct platform_device *pdev)
{
	...
	BT_SLEEP_INF("bt_wake polarity: %d", bsi->bt_wake_polarity);
	...
	BT_SLEEP_INF("host_wake polarity: %d", bsi->host_wake_polarity);
	...
	if (of_property_read_bool(np, "wakeup-source")) {
		...
		BT_ERR("device(%s) gpio %d, wakeup irq: %d\n", dev_name(dev), \
				bsi->host_wake, gpio_to_irq(bsi->host_wake));
